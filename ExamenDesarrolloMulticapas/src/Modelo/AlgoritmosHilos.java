...
public class AlgoritmosHilos extends Thread{
	...
	public void alg1() {
		...
		try {
			...
			this.icon = (new ImageIcon(new ImageIcon(imgC).getImage().getScaledInstance(200, 200, Image.SCALE_DEFAULT)));
		} catch (IOException e) {
	...
	public void alg3() {
		...
		try {
			this.conv = createImageFromConvolutionMatrix(OriImagen, img2d);
			this.imgC = ImageIO.read(conv);
			this.setIcon(new ImageIcon(new ImageIcon(imgC).getImage().getScaledInstance(200, 200, Image.SCALE_DEFAULT)));
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void alg4() {
		...
		try {
			this.conv = createImageFromConvolutionMatrix(OriImagen, img2d);
			this.imgC = ImageIO.read(conv);
			icon = (new ImageIcon(new ImageIcon(imgC).getImage().getScaledInstance(200, 200, Image.SCALE_DEFAULT)));
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
